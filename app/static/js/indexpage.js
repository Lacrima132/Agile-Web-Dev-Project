...
function growNav() {
  document.querySelector(".sidebar").style.width = "17rem";
}
    ...
    .then((data) => {
      if (data.success) {
        ...
        document.getElementById("promotionCount").innerText =
          data.promotion_count; // Update promotion count
      } else {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const printMouseCoordinates = (element) => {
    const mouseX = getComputedStyle(element).getPropertyValue("--mouse-x");
    const mouseY = getComputedStyle(element).getPropertyValue("--mouse-y");
    console.log(`--mouse-x: ${mouseX}, --mouse-y: ${mouseY}`);
  };

  for (const lb of document.querySelectorAll(".lb")) {
